...
export async function getOrganizationSeatMetrics(orgId: string) {
  // No tiers and no seat cap in a self-hosted install: there is one workspace
  // and everyone who can reach it is a full member of it. Deliberately not a
  // number — any figure here would read as a real hosted seat limit.
  return { seatsUsed: 1, seatLimit: null };
}
