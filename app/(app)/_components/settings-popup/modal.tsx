...
/**
 * Settings popup modal layer — harness `SettingsRoot.SettingsPanel`
...
/**
 * The sign-out control is the remaining eager path into the Supabase browser
 * client (`./SignOutButton` → `@/lib/supabase` → `@supabase/ssr`). It is only
 * ever rendered while the panel is open (everything below `if (!open)`), so the
 * module loads with the panel instead of with every route. The sign-out
 * behaviour itself is untouched — this only defers when the code is fetched.
 */
const SignOutButton = dynamic(() => import('./SignOutButton').then((m) => m.SignOutButton));
    ...
    return (
      <div className="lf-tokens fixed inset-0 z-[300]">
        <div className="flex h-full flex-col bg-bone text-ink">
          ...
          {/* Content — the boundary is local so a section's lazy chunk
              suspends into this slot instead of bubbling to the route
              boundary (which would blank the page behind the sheet). */}
