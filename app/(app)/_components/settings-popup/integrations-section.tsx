...
const MCP_TOOLS = [
  ...
  { name: 'archive_folio', description: 'Archive a folio: unlisted and hidden everywhere public, without deleting it. Still counts toward storage.' },
  { name: 'unarchive_folio', description: 'Restore an archived folio as a draft — it never republishes itself.' },
  ...
  { name: 'manage_seller_account', description: 'Payout account status or onboarding URL.', cloudOnly: true },
  { name: 'list_workspaces', description: 'List the workspace folders that organize folios.', cloudOnly: true },
  { name: 'manage_workspace', description: 'Create, update, delete (detaches folios), add_folio, or archive/unarchive a folder and everything in it.', cloudOnly: true },
...
export function IntegrationsSection() {
  ...
  // Two endpoints, two very different surfaces — do not conflate them:
  //   `${mcpUrl}`      the full tool set (~28 tools), OAuth-capable.
  //   `${openApiUrl}`  a hand-written REST spec covering only the five
  //                    /api/files endpoints, for clients that speak OpenAPI
  //                    and nothing else.
  // Pointing an MCP-capable client (ChatGPT, Copilot Studio, Cursor, Claude)
  // at the OpenAPI URL hands it a fraction of the product and reads as a
  // broken connector — which is exactly what happened.
  ...
  const platforms: Platform[] = [
    ...
    },
    {
      ...
      steps: [
        'Settings → Connectors → Advanced → turn on Developer mode. Custom connectors stay hidden until you do.',
        'Create → New connector, then paste the MCP server URL below.',
        'Authentication: OAuth (ChatGPT discovers our endpoints itself) or API key with your Bearer key.',
        'Enable it per conversation: + → Developer mode → pick LiveFolio. Saving the connector does NOT switch it on in existing chats.',
      ],
      snippet: mcpUrl,
      snippetLabel: 'MCP server URL',
    },
    {
      id: 'ms-copilot',
      ...
      status: isCloud ? 'Ready' : 'Cloud only',
      steps: [
        'In Copilot Studio, open your agent → Tools → Add a tool → Model Context Protocol.',
        'Choose the "Streamable" transport (SSE is no longer supported) and paste the MCP server URL below.',
        'Authentication: API key in the Authorization header, or OAuth 2.0.',
      ],
      snippet: mcpUrl,
      snippetLabel: 'MCP server URL',
    },
    {
      ...
      status: isCloud ? 'Ready' : 'Cloud only',
      steps: [
        'Where your build supports importing OpenAPI tools, paste the schema URL below and authenticate with your key.',
        'This exposes only the REST endpoints in the schema — list, get, create, update, and the design brief.',
        'For the full tool set (sharing, workspaces, review pins, archive), use an MCP client — ChatGPT, Claude, Cursor, or Copilot Studio.',
      ],
      snippet: openApiUrl,
