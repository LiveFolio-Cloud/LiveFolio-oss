...
// dynamically at the sign-out call site; a static `@/lib/supabase` import
// here put the Supabase chunk on the editor routes' critical path.
  ...
  const signOut = async () => {
    ...
    try {
      if (isCloud && isSupabaseConfigured()) {
        // Dynamic: only a signed-in cloud session ever reaches this line —
        // the SDK must not ship in the editor's first load.
