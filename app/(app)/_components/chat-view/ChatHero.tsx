...
/**
 * No-folio mode of the unified ChatView — the creation hero on
...
import {
  Send, Plus, Palette, Paperclip, Zap, RefreshCw, Globe,
...
import { HERO_DESIGN_DEFAULTS } from '@/lib/design-options';
...
export function ChatHero() {
  ...
  // shared hook (identical in FolioChatPanel).
  ...
  // Design state (hero drawer feeds the ai-create body). Defaults live in
  // lib/design-options.ts — theme MUST be a real design-system name from
  // design-systems/ (the drawer resolves these via /api/design-systems);
  // invented names get ignored by the model, which then improvises a
  // generic/brutalist look.
  const [selectedTheme, setSelectedTheme] = useState(HERO_DESIGN_DEFAULTS.theme);
  const [selectedTypography, setSelectedTypography] = useState(HERO_DESIGN_DEFAULTS.typography);
  const [selectedColorPalette, setSelectedColorPalette] = useState(HERO_DESIGN_DEFAULTS.palette);
  const [selectedLibraries, setSelectedLibraries] = useState<string[]>([...HERO_DESIGN_DEFAULTS.libraries]);
  const [isDesignDrawerOpen, setIsDesignDrawerOpen] = useState(false);

  // ── Shared plumbing ────────────────────────────────────────────────────
  // The model list, the design-system list and the Ollama probe were inline
  // copies of FolioChatPanel's. The hooks take the parts that had already
  // drifted as options; everything else is a verbatim lift.
  ...
  /** Cloud asks with the live provider + BYOK keys; OSS asks for `all_oss`. */
  ...
  const { probeOllama: checkOllamaStatus } = useOllama({
    host: ollamaHost || 'http://localhost:11434',
    selectedLocalModel,
    onAutoSelectModel: handleLocalModelChange,
    trackChecking: true,
  });
  ...
  const { availableModels, isLoadingModels, fetchAvailableModels } = useAvailableModels({
    ...
    selectedModel,
    // The hero mirrors the resolved model into its own state (the folio panel
    // leaves it to its store) — both callbacks are the plain setter here.
    onCloudDefaultModel: setSelectedModel,
    onOssModelResolved: setSelectedModel,
    errorLog: 'Failed to fetch models',
  });
  ...
  useEffect(() => {
    ...
    if (isCloud) {
      ...
      // Legacy cloud defaults are meaningless to an OSS visitor — the list is
      // shared with FolioChatPanel via lib/ai/legacy-models.ts so the two
      // surfaces can never hand out different models.
      if (isLegacyModelId(savedSelectedModel)) {
