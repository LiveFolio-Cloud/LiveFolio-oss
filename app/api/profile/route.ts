...
export async function GET() {
  try {
    ...
    if (!supabaseAdmin) {
      return err('Supabase not configured.', { status: 500 });
    }
    ...
    if (error || !profile) {
      return err('Profile not found.', { status: 404 });
    }
...
export async function PUT(request: Request) {
  try {
    ...
    if (username !== undefined && username !== null && username !== '') {
      ...
      if (claimErr) {
        ...
        return err(claimErr.message, { status: 500 });
      }
      ...
      if (!result?.success) {
        return err(result?.error || 'Failed to claim username.', { status: 409 });
      }
    }
    ...
    if (accent_color !== undefined) {
      if (accent_color === null) {
        ...
        return err('accent_color must be a 6-digit hex color like #FF3B00.', { status: 400 });
      } else {
    ...
    if (featured_folio_id !== undefined) {
      if (featured_folio_id === null || featured_folio_id === '') {
        ...
        const { data: membership } = await supabaseAdmin
          ...
          .single();
        if (!membership) {
          return err('No owned workspace found.', { status: 400 });
        }
        const { data: folio } = await supabaseAdmin
          ...
          .single();
        if (!folio) {
          return err('Folio not found or not publishable.', { status: 400 });
        }
    ...
    if (updateErr) {
      ...
      return err(updateErr.message, { status: 500 });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- error shape is dynamic (err.message may be missing)
    ...
    console.error('PUT /api/profile error:', caught.message || caught);
    return err('Internal server error', { status: 500 });
  }
}
