 ...
 * marketplace sales (payouts land on their payout account), so listings are
  ...
  try {
    ...
    if (!existing || existing.seller_terms_version !== version) {
      ...
      if (error) {
        ...
        return err('Failed to record acceptance.', { status: 500 });
      }
    }
    ...
    console.error('[POST /api/profile/attest]', caught);
