...
export async function GET(
  ...
  try {
    ...
    // Distinguishable from a missing profile on purpose: the creator exists
    // and switched their page off, and an agent relaying "Profile not found."
    // repeats a claim that is simply untrue. 404 because there is no catalog
    // to return — the page carries the human-facing explanation instead.
    if (profile.is_public === false) {
      return NextResponse.json(
        {
          error: 'PROFILE_PRIVATE',
          message: 'This creator has turned their public profile off.',
        },
        { status: 404 }
      );
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- error shape is dynamic (err.message may be missing)
    ...
    console.error('GET /api/profile/[username] error:', caught.message || caught);
