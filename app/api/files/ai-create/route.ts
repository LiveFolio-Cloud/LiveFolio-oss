...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!prompt || typeof prompt !== 'string' || !prompt.trim()) {
      return err('A non-empty prompt is required.', { status: 400 });
    }
    ...
    if (!effectiveModel) {
      return err('No AI model configured. Provide a model in the request or configure an AI provider.', { status: 400 });
    }
    ...
    if (!htmlContent) {
      return err('AI failed to generate HTML content. Please try again with a more specific prompt.', { status: 500 });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only logged
    ...
    console.error('AI folio creation error:', caught);
