...
export async function GET(
  ...
  try {
    ...
    if (isOSS) {
      ...
      const keyValid = !!project.accessKey && safeEqual((project.accessKey || '').trim(), accessKeyParam.trim());
    ...
    const isPrivate = !!project.isPrivate;
    const keyValid = !!data.access_key && safeEqual((data.access_key || '').trim(), accessKeyParam.trim());
    const hasAccess = !isPrivate || keyValid;
    ...
    if (FEATURES.paidGating && !keyAccess) {
      ...
      if (gate && !keyAccess) {
        ...
        if (user && project.organization_id) {
          isMember = await isOrgMember(user.id, project.organization_id);
        }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only logged
    ...
    console.error('GET /api/files/[id]/public error:', caught);
...
export async function POST(
  ...
  try {
    ...
    if (isOSS) {
      ...
      const isValid = serverKey !== '' && safeEqual(serverKey, sanitizedAttempt);
    ...
    if (data.archived_at) return err('Project not found', { status: 404 });
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only logged
    ...
    console.error('POST /api/files/[id]/public error:', caught);
    return err('Internal server error', { status: 500 });
  }
}
