...
/**
 * Slack / Discord notification dispatch runs detached from the HTTP response
 * (see the `setTimeout` in POST), so nothing else bounds it: a hung socket
 * would hold the timer — and its connection — open indefinitely. Same
 * `AbortSignal.timeout` idiom as `app/api/models/route.ts:47,75`.
 */
const NOTIFY_FETCH_TIMEOUT_MS = 10_000;

/**
 * The `organization_integrations` columns the notification dispatch reads. The
 * admin client is untyped, so the serial loop could read these off `any`; the
 * concurrent version needs the shape named.
 */
interface IntegrationRow {
  readonly platform: string | null;
  readonly encrypted_bot_token: string;
  readonly default_channel_id: string | null;
}
...
export async function POST(
  ...
  try {
    ...
    if (!text || !versionId || !filename) {
      return err('Missing comment parameters', { status: 400 });
    }
    ...
    if (!isOSS && !email) {
      return err('Sign in required to add comments.', { status: 401 });
    }
    ...
    if (!postLimit.allowed) {
      return err('Too many comments. Please try again later.', { status: 429 });
    }
    ...
    // Deliberately DETACHED: the timer is not awaited, so notification delivery
    // can never block or fail the HTTP response below. Only the fetches inside
    // it are made concurrent.
    ...
    setTimeout(async () => {
      try {
        ...
        // One integration row per connected platform, so the fan-out here is
        // bounded by the org's connection count (a handful at most) — no pool
        // needed. The platforms are independent: neither reads the other's
        // result and there is no aggregate. `allSettled` (not `all`) so one
        // platform's rejection cannot cancel delivery to the other; each branch
        // already logs and swallows its own errors, so the outer `catch` stays
        // as a safety net rather than the primary handler.
        await Promise.allSettled(integrations.map(async (integration: IntegrationRow) => {
...
export async function PUT(
  ...
  try {
    ...
    if (!orgId) {
      ...
      if (!resolved) return err('Unauthorized', { status: 401 });
...
export async function DELETE(
  ...
  try {
    ...
    if (!commentId) {
      return err('Missing commentId param', { status: 400 });
    }
