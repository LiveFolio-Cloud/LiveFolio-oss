...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!(await isOrgMember(userId, folioOrgId))) {
      return err('Only the folio workspace can trace copies.', { status: 403 });
    }

    const { content } = (await request.json().catch(() => ({}))) as { content?: unknown };
    if (typeof content !== 'string' || content.length === 0) {
      return err('Paste the leaked content to scan for markers.', { status: 400 });
    }
    if (content.length > 4_000_000) {
      return err('Content is too large to scan (max 4MB).', { status: 413 });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only logged
    ...
    console.error('Trace failed:', caught?.message || caught);
