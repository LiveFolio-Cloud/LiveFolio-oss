...
export async function POST(
  ...
  try {
    ...
    if (isUnsafeFolioId(id)) return err('Not found', { status: 404 });

    const filePath = request.headers.get('x-file-path');
    if (!filePath) return err('Missing x-file-path header', { status: 400 });

    const buf = Buffer.from(await request.arrayBuffer());
    if (buf.length > MAX_FILE_BYTES) {
      return err('FILE_TOO_LARGE', { status: 413 });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only caught?.message is read
  } catch (caught: any) {
    console.error(`POST /api/files/[id]/upload error:`, caught?.message || caught);
...
export async function PATCH(
  ...
  try {
    ...
    if (entries.length === 0) return err('No files', { status: 400 });

    // Total size cap — prevents memory exhaustion / DB bloat.
    let totalBytes = 0;
    for (const [, dataUrl] of entries) {
      if (typeof dataUrl !== 'string') return err('Invalid file payload', { status: 400 });
      totalBytes += Buffer.byteLength(dataUrl, 'utf8');
    }
    if (totalBytes > MAX_BATCH_TOTAL_BYTES) {
      return err('BATCH_TOO_LARGE', { status: 413 });
    }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: caught may be any thrown value; only caught?.message is read
    ...
    console.error(`PATCH /api/files/[id]/upload error:`, caught?.message || caught);
    if (caught?.message === 'Project not found') {
