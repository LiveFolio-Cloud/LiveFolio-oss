...
export async function POST(
  ...
  try {
    ...
    if (imageEntries.length === 0) {
      return err('No image files in form data', { status: 400 });
    }
    ...
    if (isOSS) {
      // Pure existence probe for the 404: readDB() hands out the shared
      // snapshot, and `.some` only reads it. The write below never touches that
      // tree — it runs inside runTransaction, whose callback receives a private
      // parse (readDBInternal) which is the only tree written back to disk.
      const snapshot = await readDB();
      if (!snapshot.some((p) => p.id === id)) {
        return err('Not found', { status: 404 });
      }

      const missing = await runTransaction(async (db) => {
        const idx = db.findIndex((p) => p.id === id);
        // Deleted between the probe and the lock. Return before mutating, so
        // the transaction's write-back is a no-op of unchanged data.
      ...
      if (missing) return err('Not found', { status: 404 });
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- catch-all handler: err may be any thrown value; err.message is read
    ...
    console.error(`POST /api/files/[id]/upload-batch error:`, error.message || error);
    return err(error.message || 'Internal error', { status: 500 });
  }
}
