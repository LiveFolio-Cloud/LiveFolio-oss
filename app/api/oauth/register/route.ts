...
// `GET` is advertised because this route also serves a GET client-lookup
// below — connectors that query their own registration need it to preflight.
export async function OPTIONS() {
  return corsPreflight({ methods: 'POST, GET, OPTIONS', headers: CORS_JSON_HEADERS, maxAge: 86400 });
}

// Wrap responses with CORS headers.
// ORIGIN ONLY, deliberately: registration responses carry a client_id and are
// same-origin-fetched by the connector that just called us; there is no
// preflight contract to honour on the response itself, and the previous
// implementation sent no Methods/Headers/Max-Age here. Left as-is.
function corsResponse(body: unknown, init?: ResponseInit) {
  return corsJson(body, init);
}
