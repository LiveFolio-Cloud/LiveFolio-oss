...
<style>
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  h1{font-family:'Space Grotesk',sans-serif;font-size:26px;font-weight:700;letter-spacing:-.035em;line-height:1.08}
...
export async function GET(request: Request) {
  ...
  if (session.claimToken) {
    const claimTokenExpired = session.claimTokenExpiresAt
      ...
      : false;
    if (!safeEqual(token, session.claimToken) || claimTokenExpired) {
      return new NextResponse(
...
export async function POST(request: Request) {
  ...
  if (authorize) {
    ...
    const claimTokenExpired = session.claimTokenExpiresAt
      ? new Date() > new Date(session.claimTokenExpiresAt)
      : false;
    if (!safeEqual(claimToken, session.claimToken) || claimTokenExpired) {
      return new NextResponse(
