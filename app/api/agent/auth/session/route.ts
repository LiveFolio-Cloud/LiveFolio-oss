...
export async function GET(request: Request) {
  ...
  if (!token || !safeEqual(token, session.preclaimToken)) {
    return NextResponse.json(
...
// GET, not POST: this endpoint is polled to read session state.
// No Max-Age on purpose (same rationale as /api/agent/auth).
export async function OPTIONS() {
  return corsPreflight({ methods: 'GET, OPTIONS', headers: CORS_JSON_HEADERS });
}
