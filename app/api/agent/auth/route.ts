...
export async function POST(request: Request) {
  try {
    ...
    if (!type) {
      return err("Missing required parameter: 'type'.", { status: 400 });
    }

    // Flow 1: Agent Verified Flow (Identity Assertion JWT / ID-JAG)
    if (type === 'identity_assertion') {
      if (!assertion) {
        return err("Missing required parameter: 'assertion' for identity_assertion type.", { status: 400 });
      }
      ...
      try {
        ...
        console.error('[Agent Auth Route] Provisioning failed:', caught);
        return err('Workspace provisioning failed. Please try again later.', { status: 500 });
      }
    }
    ...
    console.error('[Agent Auth API Error]:', caught);
    return err('Internal Server Error', { status: 500 });
  }
}

// Support pre-flight CORS requests.
// No Max-Age on purpose — this route is part of a login flow whose policy can
// change; leaving the header off keeps browsers re-validating each time.
