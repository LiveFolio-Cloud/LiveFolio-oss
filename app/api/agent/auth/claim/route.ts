...
export async function POST(request: Request) {
  try {
    ...
    if (!email) {
      return err("Missing required parameter: 'email'.", { status: 400 });
    }

    // Validate email format basic check
    if (!email.includes('@') || email.length < 5) {
      return err("Invalid email address format.", { status: 400 });
    }
    ...
    if (!updated) {
      return err("Failed to update registration session.", { status: 500 });
    }
    ...
    console.error('[Agent Auth Claim Error]:', caught);
