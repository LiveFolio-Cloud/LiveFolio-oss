 ...
 *
 * Executes a validated tool call server-side.  Supports both
...
import { assertWebToolPlan } from '@/lib/ai/quota-gate';
...
export async function POST(request: Request) {
  try {
    ...
    // Plan gating: web_search and web_fetch require Team or higher.
    // Fail CLOSED: if the org context or plan lookup fails, the tools are
    // denied rather than silently granted to Free users. The gate is shared
    // with the editor tool loop and the Slack/Discord tool loop
    // (lib/ai/quota-gate) so all three paths resolve the plan identically.
    const webGate = await assertWebToolPlan({ toolName, orgId });
    if (!webGate.ok) {
      return NextResponse.json(webGate.body, { status: webGate.status });
    }
    ...
    const result = await runTransaction(async (db) => {
      ...
      if (toolResult.files && toolResult.type === 'files') {
        const newVersionId = nextFolioVersionId(project.versions);
        ...
        project.versions = await applyVersionRetention(updatedVersions, orgId);
        toolResult.versionId = newVersionId;
      }
