...
export async function POST(request: Request) {
  ...
  if (!perUser.allowed) {
    return err('Too many test emails. Please try again later.', { status: 429 });
  }
  const perIp = checkRateLimit(`email-test:ip:${clientIp}`, 20, 60 * 60_000, 24 * 60 * 60_000);
  if (!perIp.allowed) {
    return err('Too many test emails from this network.', { status: 429 });
  }
  ...
  try {
    ...
    return err('Invalid JSON body. Provide: { "to": "email@example.com" }', { status: 400 });
  }

  if (!toEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(toEmail)) {
    return err('Please provide a valid "to" email address.', { status: 400 });
  }
