...
export async function POST(
  ...
  try {
    ...
    if (!folioId) {
      return err('folioId is required.', { status: 400 });
    }
    ...
    if (folioError || !folio) {
      return err('Folio not found or does not belong to this organization.', { status: 404 });
    }
    ...
    return err(error.message || 'Failed to add folio to project.', { status: 500 });
  }
}
