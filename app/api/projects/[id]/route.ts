...
export async function PUT(
  ...
  try {
    ...
    if (name !== undefined) {
      if (!name.trim()) {
        return err('Name cannot be empty.', { status: 400 });
      }
    ...
    if (paid_access !== undefined) {
      if (paid_access === null) {
        ...
        if (!sanitized) {
          return err('Invalid paid_access configuration.', { status: 400 });
        }
    ...
    if (thumbnail_url !== undefined) {
      if (thumbnail_url === null) {
        ...
        return err('Invalid thumbnail_url.', { status: 400 });
      }
    }
    ...
    if (providedSlug !== undefined) {
      ...
      if (!slug) {
        return err('Could not generate a valid slug from the provided name.', { status: 400 });
      }
      ...
      if (existing) {
        return err(`A project with the slug "${slug}" already exists in this organization.`, { status: 409 });
      }
