...
import { parsePinFocusParam } from '@/lib/inbox';
...
// Flag only — the dependency-free probe. The SDK itself is imported
// dynamically at the (paid-gate) call site; a static `@/lib/supabase` import
// here put ~65 KB gzip of @supabase/ssr on the share viewer's critical path.
...
export default function GuestPresentationPage({ initialProject }: { initialProject?: InitialShareData | null }) {
  ...
  // ── Inbox deep-link: /share/<slug>?pin=<commentId> ────────────────────
  // The Inbox links straight to the feedback it is telling you about. Read the
  // param once at mount (landing behaviour, not a reactive route) and fire the
  // focus when the in-folio bridge reports ready.
  //
  // Ordering here is deliberately not delicate: the bridge parks a focus that
  // arrives before its pins, so it does not matter whether this lands before or
  // after the PINS_SYNC above.
  const pinFocusIdRef = useRef<string | null>(null);
  useEffect(() => {
    // Parsing lives in lib/inbox.ts (`parsePinFocusParam`) so the inverse of
    // the href builder that CREATES these links is testable in one place.
    pinFocusIdRef.current = parsePinFocusParam(window.location.href);
  }, []);

  useEffect(() => {
    if (!bridgeReady) return;
    const pinId = pinFocusIdRef.current;
    if (!pinId) return;
    // Clear before posting: bridgeReady also flips on page/version changes, and
    // the focus is a one-shot for the initial landing.
    pinFocusIdRef.current = null;
    iframeRef.current?.contentWindow?.postMessage({ type: 'LIVEFOLIO_PIN_FOCUS', pinId }, '*');
  }, [bridgeReady]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Dynamic: this is the only Supabase call on the page, and only gated
        // folios reach it — the SDK must not ship in the viewer's first load.
      ...
      >
        ...
        <div className="mx-4 mt-3 flex h-12 items-center justify-between gap-2 rounded-xl bg-white/85 px-3 shadow-sm ring-1 ring-black/5 backdrop-blur-md">
          ...
          <div className="flex items-center gap-1 shrink-0 ml-2">
            ...
            <span className="font-black text-sm tracking-tighter" style={{ fontFamily: FONT_DISPLAY }}>LiveFolio</span>
