...
/**
 * Turns a `listingWriteGuard` code into the HTTP response.
 *
 * Every write surface (REST files create/update, ai-create, MCP, Slack/Discord)
 * had its own copy of the same three-branch ladder. The ladder IS the contract,
 * so it lives here rather than in five call sites:
 *
 * - `LISTING_NEEDS_SELLER_AGREEMENT` / `LISTING_SELLER_SUSPENDED` → 403
 *   (the org owner's account standing blocks sales)
 * - `LISTING_NEEDS_RIGHTS_ATTESTATION` → 400 (the request itself is incomplete)
 * - `null` (guard passed) → null, meaning "no response, carry on"
 *
 * The code is echoed verbatim as `error` — clients and agents switch on it.
 */
export function listingWriteGuardResponse(code: ListingWriteErrorCode | null): NextResponse | null {
  // Order and fall-through mirror the hand-written ladders exactly: a code
  // that is neither of these two groups produces NO response, so the write
  // proceeds — adding a future code must not silently start rejecting writes.
  if (code === 'LISTING_NEEDS_SELLER_AGREEMENT' || code === 'LISTING_SELLER_SUSPENDED') {
    return NextResponse.json({ error: code }, { status: 403 });
  }
  if (code === 'LISTING_NEEDS_RIGHTS_ATTESTATION') {
 ...
 *    land on their payout account) must have accepted the Seller Terms,
