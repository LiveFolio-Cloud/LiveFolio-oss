...
//
// The validate route (`app/api/chat/tools/validate/route.ts`) pre-checks the
// same preconditions these executors enforce on the way out. The *decision* is
// defined here exactly once so the two can never drift; the user-visible
// wording stays at each call site, because the two differ (the validate
// route's strings are what the UI shows, the executors' are what the AI reads).
//
// Parameters are typed for the executor's own arguments. The validate route
// passes untyped `toolArguments` through the same guards the executor applies,
// so both sides evaluate these under identical conditions.

/** edit_element — `selector` is missing (required argument). */
export function editElementSelectorMissing(selector: unknown): boolean {
  return !selector;
}

/** edit_element — `new_html` is missing (required argument). */
export function editElementNewHtmlMissing(newHtml: unknown): boolean {
  return !newHtml;
}

/**
 * The named file is not in the current version.
 *
 * Shared by edit_element, delete_page and export_folio (its target file,
 * `filename || 'index.html'`, is always present).
 */
...
/**
 * add_page — the filename does not end in ".html".
 *
 * Only reached with a filename present: the validate route guards `!filename`
 * first, and the executor's arguments are typed `filename: string`.
 */
export function addPageBadSuffix(filename: string): boolean {
  return !filename.endsWith('.html');
}

/** add_page — a page with this filename already exists. */
export function addPageExists(
  currentFiles: { [filename: string]: string },
  filename: string | undefined
): boolean {
  return !!filename && !!currentFiles[filename];
}

/** delete_page — the folio has one or fewer HTML files, so nothing can be deleted. */
export function isLastHtmlPage(currentFiles: { [filename: string]: string }): boolean {
  return Object.keys(currentFiles).filter((k) => k.endsWith('.html')).length <= 1;
}

/**
 * export_folio — `format` is anything other than "pdf".
 *
 * Only reached with a format present: the validate route guards `!format`
 * first (reporting "format is required"), and the executor's argument is typed
 * `format: string`.
 */
export function exportFormatUnsupported(format: string): boolean {
  return format !== 'pdf';
}
...
export function executeEditElement(
  ...
  if (fileMissing(ctx.currentFiles, filename)) {
    return { type: 'text', message: `File "${filename}" not found in the current version.` };
  }

  if (editElementSelectorMissing(selector) || editElementNewHtmlMissing(new_html)) {
...
export function executeAddPage(
  ...
  if (addPageBadSuffix(filename)) {
    return { type: 'text', message: 'Filename must end with ".html".' };
  }

  if (addPageExists(ctx.currentFiles, filename)) {
...
export function executeDeletePage(
  ...
  if (isLastHtmlPage(ctx.currentFiles)) {
...
export function executeExportFolio(
  ...
  if (exportFormatUnsupported(format)) {
  ...
  if (fileMissing(ctx.currentFiles, targetFile)) {
