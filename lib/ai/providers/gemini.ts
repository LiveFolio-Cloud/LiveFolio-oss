  ...
  async generateCompletion(
    ...
    const ai = new GoogleGenAI({
      ...
      httpOptions: {
        ...
        // The SDK aborts each request itself via AbortController once this
        // elapses (see @google/genai includeExtraHttpOptionsToRequestInit).
        // Gemini emits up to 16384 output tokens across two sequential calls,
        // so this is per-request budget, not a per-generation one.
        timeout: 180000
      }
    });
...
`;
  ...
  async generateCompletionWithTools(
    ...
    const ai = new GoogleGenAI({
      apiKey,
      httpOptions: {
        headers: { 'User-Agent': 'LiveFolio-oss-client' },
        timeout: 180000,
      },
    });
