...
export async function suggestHandles(userId: string, count = 3): Promise<string[]> {
  ...
  // Left SERIAL, deliberately: this is a short-circuiting ordered search, not
  // a fan-out. `is_username_available` is the entire cost, and both loops stop
  // early on a data-dependent condition — the outer at `candidates.length >=
  // count`, the inner at the same count while walking the nine variants. Any
  // concurrent version would have to issue probes this version deliberately
  // skips (a bounded pool cannot know the answer to probe 1 before claiming
  // probe 2), which changes the call set for no gain in the common case: seeds
  // number 2-3, and the expensive nine-variant branch only runs when the sole
  // seed is taken. Restructuring the cascade into a fixed probe wave would be
