#!/usr/bin/env node
/**
 * Standalone release build (workstream B: one-command OSS install).
 *
...
// Request-body clamp. Next's auto-generated standalone server bakes
...
// Stale-.next guard: a default `next build` before a packager run can
// leave a stale trace that fails the packager mid-stage ("missing
// status/page.js"). Always build from a clean .next/.
...
try {
  ...
  // server.js cannot. Live-confirmed: an 11 MB folio create →
...
# LiveFolio standalone boot (one-command OSS install).
#
# Environment:
...
// Verify the 50 MB body clamp actually shipped inside the tarball
