 ...
 *                      LIVEFOLIO_PIN_FOCUS     {pinId}  — scroll to one pin and
 *                                               highlight it (Inbox deep-links:
 *                                               /share/<slug>?pin=<commentId>)
 *
...
(function () {
  ...
  var state = {
    ...
    accent: '#FF3B00',
    mode: false,
    // A focus request that arrived before its pins did. The parent may send
    // FOCUS in the same tick as the sync (or the pin may live on a page whose
    // markers are not built yet), so the request is parked here and applied at
    // the end of the next render instead of being dropped.
    pendingFocusId: null,
  };
  ...
  // ── Deep-link focus (LIVEFOLIO_PIN_FOCUS) ─────────────────────────────

  // Injected once per document. The ring is drawn as a sibling INSIDE the
  // marker so it inherits its stacking context and cannot be clipped by
  // anything the folio does to its own layers.
  function ensureFocusStyles() {
    if (document.getElementById('livefolio-pin-focus-styles')) return;
    var style = document.createElement('style');
    style.id = 'livefolio-pin-focus-styles';
    style.textContent =
      '@keyframes livefolioPinPulse{' +
      '0%{transform:scale(0.7);opacity:0.85}' +
      '80%{transform:scale(2.6);opacity:0}' +
      '100%{transform:scale(2.6);opacity:0}' +
      '}' +
      '.livefolio-pin-focus-ring{position:absolute;inset:-7px;border-radius:9999px;' +
      'border:2px solid ' + state.accent + ';pointer-events:none;' +
      'animation:livefolioPinPulse 1.5s ease-out 3;}';
    (document.head || document.documentElement).appendChild(style);
  }

  var focusResetTimer = null;

  /**
   * Bring one pin into view and make it unmistakable.
   *
   * Silent no-op for an id this version does not contain — a deep-link into a
   * version where the pin was deleted (or an id from the editor) must not throw
   * or show an error on a shared page.
   */
  function focusPin(pinId) {
    if (!pinId || !state.pins || !state.pins.length) return false;
    var idx = -1;
    for (var i = 0; i < state.pins.length; i++) {
      if (String(state.pins[i].id) === String(pinId)) { idx = i; break; }
    }
    if (idx === -1) return false; // stale id — silently ignore

    if (!markers.length) renderMarkers(); // markers are built by render(); a focus can arrive first
    var marker = markers[idx];
    if (!marker) return false;

    var xy = markerXY(state.pins[idx]);
    var targetTop = Math.max(0, xy.y - window.innerHeight / 2);
    try {
      window.scrollTo({ top: targetTop, behavior: 'smooth' });
    } catch (err) {
      window.scrollTo(0, targetTop); // older engines without options
    }

    ensureFocusStyles();

    var ring = document.createElement('div');
    ring.className = 'livefolio-pin-focus-ring';
    marker.appendChild(ring);

    var inner = marker.children[0];
    var tooltip = marker.children[1];
    // Open the tooltip so the reader sees WHICH feedback they were linked to,
    // and keep it open long enough to read — this is the one moment the pin's
    // text is the point of the visit.
    if (tooltip) tooltip.style.display = 'block';
    if (inner) {
      inner.style.transform = 'scale(1.25)';
      inner.style.boxShadow = '0 0 0 3px rgba(255,59,0,0.28), 0 6px 20px rgba(15,15,13,0.35)';
    }

    if (focusResetTimer) clearTimeout(focusResetTimer);
    focusResetTimer = setTimeout(function () {
      if (ring.parentNode) ring.parentNode.removeChild(ring);
      if (inner) {
        inner.style.transform = '';
        inner.style.boxShadow = '';
      }
      // Only re-hide the tooltip if the pointer is not resting on the marker —
      // collapsing it under an active hover would look broken.
      if (tooltip && !marker.matches(':hover')) tooltip.style.display = 'none';
    }, 4600);
  ...
  function render() {
    ...
    renderMarkers();
    setTint(state.mode);
    // A focus that arrived before the pins/markers were ready is applied here,
    // once, and then cleared.
    if (state.pendingFocusId) {
      var id = state.pendingFocusId;
      if (focusPin(id)) state.pendingFocusId = null;
    }
  }
  ...
  window.addEventListener('message', function (e) {
    ...
    if (d.type === 'LIVEFOLIO_PINS_SYNC') {
      ...
      setMode(!!d.active);
    } else if (d.type === 'LIVEFOLIO_PIN_FOCUS') {
      // Park it if the pins are not here yet; render() picks it up.
      if (!focusPin(d.pinId)) state.pendingFocusId = d.pinId || null;
