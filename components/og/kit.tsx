...
import { FONT_DISPLAY_OG } from '@/lib/fonts';
...
/**
 * Font stack for the OG renderers. Canonical value lives in `lib/fonts.ts`
 * (`FONT_DISPLAY_OG`) so there is exactly one definition of the brand stack
 * in the codebase. Cabinet Grotesk stays first here on purpose: the OG
 * routes embed it server-side via Fontshare (`loadBrandFont`), so unlike the
 * web surfaces it is not a dead reference.
 */
export const FONT_DISPLAY = FONT_DISPLAY_OG;
