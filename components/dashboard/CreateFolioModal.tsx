 ...
 * Tab 2 "With AI" — skeleton only in this phase.
 *                   Gated at runtime in OSS (/api/files/ai-create is
 *                   stripped from the OSS build).
 *
...
//
// Every name here MUST have a folder in `design-systems/`, because the theme is
// saved onto the folio and later resolved against that directory (the editor
// chat paths load its DESIGN.md as a strict spec). Two entries used to resolve
// to nothing — a folio created with them styled as nothing at all:
//   'Night Emerald' -> not a design system at all; it is one of the six
//                      PALETTE names in lib/ai/prompt-builder.ts.
//   'Minimalist'    -> the folder is named `minimal`.
// Both were replaced with real systems of matching intent.
const THEME_OPTIONS = [
  'Cosmic',        // dark sci-fi with neon accents — replaces 'Night Emerald'
  'Minimal',       // was 'Minimalist' (no such folder)
  'Brutalism',
  'Bento',
  'Editorial',
  'Glassmorphism',
...
// NOTE: this list must stay identical to DesignDrawer's TYPOGRAPHY_PAIRINGS.
// 'Cabinet Grotesk & Inter' was removed from both — Cabinet Grotesk has no font
// file in this app and its CDN is not in the CSP, so it could never render.
// 'Space Grotesk & Inter' replaces it and loads from Google Fonts.
